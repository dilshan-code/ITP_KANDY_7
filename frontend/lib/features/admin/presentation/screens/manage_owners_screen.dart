...
class _ManageOwnersScreenState extends State<ManageOwnersScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: adminProvider.isLoading && adminProvider.owners.isEmpty
  ...
  List<Owner> _buildFilteredOwners(List<Owner> owners) {
    ...
    final filtered = owners.where((owner) {
      final matchesFilter = switch (_selectedFilter) {
        'pending' => owner.status == 'pending' && owner.isSuspended == false,
        'approved' =>
          owner.status == 'approved' && owner.isSuspended == false,
        'suspended' =>
  ...
  int _statusRank(Owner owner) {
    if (owner.isSuspended || owner.status == 'suspended') return 2;
    if (owner.status == 'pending') return 0;
    return 1;
  }
}
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Review new registrations, approve trusted owners, suspend risky accounts, and keep every partner profile accurate.',
            style: TextStyle(
          ...
          Row(
            children: [
              Expanded(
                child: _MiniStat(
                  label: 'Pending',
                  value: provider.pendingOwners.toString(),
                  tint: const Color(0xFFFFE8A3),
                ),
              ),
              ...
              Expanded(
                child: _MiniStat(
                  label: 'Approved',
                  value: provider.approvedOwners.toString(),
...
class _StatusFilters extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final filters = [
      ...
      ('pending', 'Pending', provider.pendingOwners),
      ('approved', 'Approved', provider.approvedOwners),
...
class _EmptyOwnersState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final text = switch (activeFilter) {
      ...
      'approved' => 'No approved owner accounts match your search.',
      'suspended' => 'No suspended owners found.',
      _ => 'No owner registrations found.',
    };
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              runSpacing: 10,
              children: [
                if (owner.status != 'approved' || owner.isSuspended)
                  _ActionButton(
                    label: 'Approve',
                    ...
                    onPressed: () => _runAction(
                      context,
                      () => context.read<AdminProvider>().approveOwner(owner.id),
                ...
                if (owner.status != 'pending' || owner.isSuspended)
                  _ActionButton(
                    label: 'Set Pending',
                    ...
                    onPressed: () => _runAction(
                      context,
                      () => context.read<AdminProvider>().moveOwnerToPending(
                        owner.id,
                      ),
                      'Owner moved back to pending review',
                    ),
                  ),
                _ActionButton(
                  label: 'Edit Details',
...
class _OwnerEditSheetState extends State<_OwnerEditSheet> {
  ...
  void initState() {
    ...
    _selectedStatus = widget.owner.status;
    _isSuspended =
        widget.owner.isSuspended || widget.owner.status == 'suspended';
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Edit registration details, adjust approval state, or restore access from one place.',
                  style: TextStyle(
                    height: 1.4,
                    color: Color(0xFF5E7A73),
                  ),
                ...
                DropdownButtonFormField<String>(
                  value: _isSuspended ? 'suspended' : _selectedStatus,
                  items: const [
                    DropdownMenuItem(
                      value: 'pending',
                ...
                SwitchListTile.adaptive(
                  ...
                  subtitle: const Text(
                    'Suspended owners cannot log in until restored.',
                  ),
                  value: _isSuspended,
  ...
  Future<void> _save(BuildContext context) async {
    ...
    final payload = {
      ...
      'status': _isSuspended ? 'suspended' : _selectedStatus,
      'isSuspended': _isSuspended,
    };

    final success = await adminProvider.updateOwner(widget.owner.id, payload);
    if (!context.mounted) return;
