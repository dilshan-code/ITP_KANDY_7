...
class _EditProductScreenState extends State<EditProductScreen> {
  ...
  void initState() {
    ...
    _costPriceController = TextEditingController(text: widget.product.costPrice.toStringAsFixed(2));
    _minStockController = TextEditingController(text: widget.product.minimumStockLevel.toString());
  ...
  Future<void> _updateProduct() async {
    ...
    final success = await context.read<ProductProvider>().updateProduct(widget.product.id, data);
    ...
    if (mounted) {
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to update product'), backgroundColor: AppColors.error),
        );
      }
    }
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildPriceField('SELLING PRICE', _sellingPriceController)),
                const SizedBox(width: 16),
                Expanded(child: _buildPriceField('COST PRICE', _costPriceController)),
              ],
            ),
            const SizedBox(height: 20),
            // Stock quantity
            ...
            const SizedBox(height: 8),
            _buildFormField(_minStockController, keyboardType: TextInputType.number),
            const SizedBox(height: 20),
            // Description
            _buildLabel('DESCRIPTION'),
            const SizedBox(height: 8),
            _buildFormField(_descriptionController, maxLines: 4, hint: 'Enter product description...'),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _saving
                    ...
                    : const Text('Update Product', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
              ),
            ),
    ...
    return Column(
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            prefixText: 'Rs.  ',
            prefixStyle: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.textMedium),
            filled: true,
            fillColor: AppColors.primary.withValues(alpha: 0.04),
            border: OutlineInputBorder(
              borderRadius: BorderRadius.circular(10),
